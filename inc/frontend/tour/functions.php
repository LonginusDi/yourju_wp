...
if ( ! function_exists( 'ct_tour_generate_conf_mail' ) ) {
	function ct_tour_generate_conf_mail( $order, $type='new' ) {
		...
		if ( ! empty( $order_data ) ) {
			...
			$tour_address = get_post_meta( $order_data['tour_id'], 'ct_tour_address', true );
			$tour_email = get_post_meta( $order_data['tour_id'], 'ct_tour_email', true );
			$tour_phone = get_post_meta( $order_data['tour_id'], 'ct_tour_phone', true );
...
if ( ! function_exists( 'ct_tour_get_special_tours' ) ) {
	function ct_tour_get_special_tours( $type='latest', $count=6, $exclude_ids, $tour_type=array() ) {
		...
		if ( $type == 'featured'  ) {
			...
			$sql = 'SELECT tour_id, COUNT(*) AS booking_count FROM ' . CT_TOUR_BOOKINGS_TABLE . ' AS booking
